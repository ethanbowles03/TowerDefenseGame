...
public class EnemyHaasTruck extends Enemy {
	...
	public EnemyHaasTruck(double p, GameState state) {
		super(p);
		degree = 0;
		anchorX = 0;
		anchorY = 0;
	...
	public void update(double timeElapsed) {
		percentTravelled = percentTravelled + 0.04 * timeElapsed;
	...
	public void draw(Graphics g, GameView v) {
		...
		if (percentTravelled < 1) {
			...
			BufferedImage carImage = ResourceLoader.getLoader().getImage("HaasTruck.png");
			BufferedImage car = rotateIcon(carImage, degree - 90);

			// Find the center point where the car is located and then draws image
			int xPos = (int) (objPoint.getX() - anchorX - 10);
			int yPos = (int) (objPoint.getY() - anchorY - 30);
